...
%         [fieldSize,field_idx,peak_time_variance,activation_probability,firing_field] 
       [fieldSize,field_idx, peak_time_variance,activation_probability,firing_field]...
           = single_cell_field_characteristics(mData(load_data(i),f).(whichSignal).rmapsAA,mData(load_data(i),f).(whichSignal).rmapsBA,TimeA);
       ...
       % odor A
       [fieldSize,field_idx, peak_time_variance,activation_probability,firing_field]...
           = single_cell_field_characteristics(mData(load_data(i),f).(whichSignal).rmapsAA,mData(load_data(i),f).(whichSignal).rmapsBA,OdorA);
       ...
       [fieldSize,field_idx, peak_time_variance,activation_probability,firing_field]...
           = single_cell_field_characteristics(mData(load_data(i),f).(whichSignal).rmapsAB,mData(load_data(i),f).(whichSignal).rmapsBB,TimeB);
       ...
       [fieldSize,field_idx, peak_time_variance,activation_probability,firing_field]...
           = single_cell_field_characteristics(mData(load_data(i),f).(whichSignal).rmapsAB,mData(load_data(i),f).(whichSignal).rmapsBB,OdorB);
...
%         field_idx_trial = NaN(length(activeTrials),1);
%         m
         [~,field_idx_trial] = max(rmap');
%         for k = 1:length(activeTrials)
% %            [~,field_idx_trial(k)] = max(rmap(activeTrials(k),firing_field{m}));
%             field_idx_trial(k)    = analysis.find_firing_field(lowpass(rmap(activeTrials(k),firing_field{m}),Fpass,Fs));
%         end


                % Convert data points to radians
%         theta = (field_idx_trial*0.161 / 16) * 2 * pi;
% 
...
%         peak_time_variance(m) = 1 - R;

        % Display the result
%         fprintf('The circular variance is %.4f\n', V);
...
% %         end
%         
%         peak_time_variance(m)     = nanvar(field_idx_trial*0.1579);
%       
%     else
%         activation_probability(m) = NaN;
%         peak_time_variance(m)     = NaN;
