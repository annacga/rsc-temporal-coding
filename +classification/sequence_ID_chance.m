function mData = sequence_ID_chance(mData,data, varargin)
...
whichSignal = 'deconv';
savedir = '/Users/annachristinagarvert/UIO Physiology Dropbox Dropbox/Lab Data/Malte Bieler/Fig_1/classification_chance_include_ITI';
for i = load_data
    for f = 1:length(data(i).sessionIDs)
        
        ClassifiedCells = mData(i,f).classification_chance;
        ...
        for m = 1:length(TimeAB)
            ...
            mean_rmapsA = nanmean(ClassifiedCells{TimeAB(m),4}(:,1:44));
            mean_rmapsB = nanmean(ClassifiedCells{TimeAB(m),8}(:,1:44));
        ...
        for m = 1:length(OdorAB)
            ...
            mean_rmapsA = nanmean(ClassifiedCells{OdorAB(m),4}(:,1:44));
            mean_rmapsB = nanmean(ClassifiedCells{OdorAB(m),8}(:,1:44));
            % find firing field f
            [firing_field_A,indices_A] = analysis.find_firing_field(lowpass(mean_rmapsA,Fpass,Fs));
            [firing_field_B,indices_B] = analysis.find_firing_field(lowpass(mean_rmapsB,Fpass,Fs));
        ...
        for m = 1:length(TimeA)
            ...
            mean_rmapsA = nanmean(ClassifiedCells{TimeA(m),4}(:,1:44));
            mean_rmapsB = nanmean(ClassifiedCells{TimeA(m),8}(:,1:44));
        ...
        for m = 1:length(TimeB)
            ...
            mean_rmapsA = nanmean(ClassifiedCells{TimeB(m),4}(:,1:44));
            mean_rmapsB = nanmean(ClassifiedCells{TimeB(m),8}(:,1:44));
        ...
        for m = 1:length(OdorA)
            ...
            mean_rmapsA = nanmean(ClassifiedCells{OdorA(m),4}(:,1:44));
            mean_rmapsB = nanmean(ClassifiedCells{OdorA(m),8}(:,1:44));
        ...
        for m = 1:length(OdorB)
            ...
            mean_rmapsA = nanmean(ClassifiedCells{OdorB(m),4}(:,1:44));
            mean_rmapsB = nanmean(ClassifiedCells{OdorB(m),8}(:,1:44));
        ...
        mData(i,f).TimeA_chance.SI  = SI_timeA;
        mData(i,f).TimeB_chance.SI  = SI_timeB;
        ...
        mData(i,f).OdorA_chance.SI  = SI_odorA;
        mData(i,f).OdorB_chance.SI  = SI_odorB;
